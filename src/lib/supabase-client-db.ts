...
// Environment variables are injected at build-time by Vite defineConfig define block
  ...
  if (client) {
    try {
      ...
      if (dbUser) {
        ...
        let { error } = await client.from("users").upsert(payload);
        if (error && (error.message?.includes("must_change_password") || error.message?.includes("column"))) {
          delete payload.must_change_password;
          const retry = await client.from("users").upsert(payload);
          error = retry.error;
        }
        return !error;
      }
